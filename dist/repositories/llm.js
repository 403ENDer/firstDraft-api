...
class LLM {
    ...
    cleanText(text) {
        ...
        if (jsonMatch) {
            ...
            if (jsonMatches && jsonMatches.length > 0) {
                ...
                text = jsonMatches.reduce((largest, current) => current && largest && current.length > largest.length
                    ? current
                    : largest);
            }
        }
    ...
    reconstructJSON(text) {
        try {
            ...
            if (text.includes('"scenes"') || text.includes("scenes")) {
                ...
                const sceneMatches = text.match(/"scene_number":\s*(\d+)[^}]*"content":\s*"([^"]*(?:[^\\]"[^"]*)*)"/g);
    ...
    async classifyInput(userInput) {
        const template = new prompts_1.PromptTemplate({
...
{{
  ...
  "narrative_design": {{
    ...
    /** Tech Layer Agent */
    async generateTechLayer(feature_analysis, user_prompt) {
        const template = new prompts_1.PromptTemplate({
...
`,
    ...
    async generateScreenplay(chunks) {
        const template = new prompts_1.PromptTemplate({
...
{chunks}
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    console.error("Third JSON parsing attempt (reconstruction) failed:", thirdErr);
    ...
    async generatePreviousResponse(sessionId, userInput) {
        const chatHistory = await ChatSession_1.ChatSession.findOne({ sessionId });
        if (!chatHistory) {
        ...
        const contents = [...historyContents, new messages_1.HumanMessage(userInput)];
        const model = this.getModel();
    ...
    /** Main workflow */
    async processUserInput(userInput, sessionId) {
        ...
        if (classification === "General Conversation") {
            const response = await this.generatePreviousResponse(sessionId, userInput);
            return {
            ...
            const techLayer = await this.generateTechLayer(featureAnalysis, userInput);
            ...
            if ("error" in screenplay) {
                ...
                return {
                    ...
                    result: {
                        message: "I've analyzed your request and created a structured breakdown. However, I encountered an issue generating the final screenplay format. Here's what I was able to process:",
